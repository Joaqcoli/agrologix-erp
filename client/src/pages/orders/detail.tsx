...
export default function OrderDetailPage({ id }: { id: number }) {
  ...
  const [waOption, setWaOption] = useState<"remito" | "factura" | "ambos">("remito");
  ...
  const handleWaSend = async () => {
    if (!order) return;
    const rawPhone = (order.customer as any).phone ?? "";
    ...
    try {
      if (waOption === "remito" || waOption === "ambos") {
        await handleDownloadRemito();
      }
      if (waOption === "factura" || waOption === "ambos") {
        let iid = emittedInvoice?.id ?? null;
        if (!iid) {
          const lst = await fetch(`/api/invoices?orderId=${id}`, { credentials: "include" }).then((r) => r.json());
          iid = lst?.[0]?.id ?? null;
        }
        if (iid) {
      ...
      <Dialog open={waDialog} onOpenChange={setWaDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          {!(order?.customer as any)?.phone ? (
            <Alert>
              <AlertDescription>El cliente no tiene número de teléfono registrado.</AlertDescription>
            </Alert>
          ) : (
            <div className="space-y-4 py-1">
              <div className="space-y-2">
                ...
                <div className="flex flex-col gap-2">
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <input type="radio" name="waOpt" value="remito" checked={waOption === "remito"} onChange={() => setWaOption("remito")} />
                    Solo Remito
                  </label>
                  {(order?.invoiceNumber || emittedInvoice) && (
                    <>
                      <label className="flex items-center gap-2 text-sm cursor-pointer">
                        <input type="radio" name="waOpt" value="factura" checked={waOption === "factura"} onChange={() => setWaOption("factura")} />
                        Solo Factura
                      </label>
                      <label className="flex items-center gap-2 text-sm cursor-pointer">
                        <input type="radio" name="waOpt" value="ambos" checked={waOption === "ambos"} onChange={() => setWaOption("ambos")} />
                        Remito y Factura
                      </label>
                    </>
                  )}
              ...
              <p className="text-xs text-muted-foreground">Teléfono: {(order?.customer as any)?.phone}</p>
            </div>
          )}
          <DialogFooter>
