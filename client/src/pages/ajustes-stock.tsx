...
export default function AjustesStockPage() {
  ...
  const filtered = useMemo(() => rows.filter((r) => {
    ...
    return true;
  }), [rows, search, tipoFilter, whoFilter, dateFilter]);

  const pre = filtered.filter((r) => r.section === "pre");
  const post = filtered.filter((r) => r.section === "post");
  const totalPerdido = filtered.reduce((s, r) => s + Math.min(0, r.value ?? 0), 0);
  const totalGanado = filtered.reduce((s, r) => s + Math.max(0, r.value ?? 0), 0);
          ...
          <tbody>
            {items.map((a) => (
              <tr key={a.id} className="border-b last:border-0 hover:bg-muted/40" data-testid={`adj-row-${a.id}`}>
                ...
                <td className={`py-2 pr-3 text-right tabular-nums font-semibold ${(a.value ?? 0) < 0 ? "text-red-600" : "text-green-600"}`}>
                ...
                <td className="py-2 text-right">
                  {a.revertible ? (
                    ...
                    <span className="text-[10px] text-muted-foreground inline-flex items-center gap-1" title="El rinde de un pedido se maneja editando/des-aprobando el pedido">
    ...
    <Layout title="Ajustes de Stock">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
        <Card><CardContent className="p-4">
          <p className="text-xs text-muted-foreground">Perdido (merma / peso)</p>
          <p className="text-2xl font-bold text-red-600">{fmtMoney(totalPerdido)}</p>
        </CardContent></Card>
        <Card><CardContent className="p-4">
          <p className="text-xs text-muted-foreground">Ganado / apareció (rinde)</p>
          <p className="text-2xl font-bold text-green-600">{fmtMoney(totalGanado)}</p>
        </CardContent></Card>
        <Card><CardContent className="p-4">
          <p className="text-xs text-muted-foreground">Neto con mercadería</p>
          <p className={`text-2xl font-bold ${neto < 0 ? "text-red-600" : "text-green-600"}`}>{fmtMoney(neto)}</p>
        </CardContent></Card>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        <Select value={whoFilter} onValueChange={setWhoFilter}>
          <SelectTrigger className="w-40"><SelectValue placeholder="Quién" /></SelectTrigger>
          <SelectContent>
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h2 className="font-semibold">Pre-venta — ajustes de peso del galpón</h2>
                <Badge variant="outline" className="text-[10px]">{pre.length}</Badge>
              </div>
              <p className="text-xs text-muted-foreground mb-2">Correcciones de kilaje por envase antes de vender (a tener en cuenta para próximas compras).</p>
              {renderTable(pre, "Sin ajustes de peso en este filtro.")}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <p className="text-xs text-muted-foreground mb-2">Lo que se perdió (merma) o apareció (rinde) — manual y de pedidos.</p>
              {renderTable(post, "Sin merma/rinde en este filtro.")}
            </CardContent>
          </Card>
        </div>
      )}
